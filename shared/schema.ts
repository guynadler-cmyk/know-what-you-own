...
export const aiBusinessAnalysis = pgTable(
  ...
  (table) => ({
    cacheKeyIdx: index("idx_business_cache_key").on(table.cacheKey),
    tickerYearIdx: index("idx_business_ticker_year").on(
      table.ticker,
      table.fiscalYear
    ),
  })
...
export const aiTemporalAnalysis = pgTable(
  "ai_temporal_analysis",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    cacheKey: text("cache_key").notNull().unique(),
    companyName: text("company_name").notNull(),
  ...
  (table) => ({
    cacheKeyIdx: index("idx_temporal_cache_key").on(table.cacheKey),
    tickerIdx: index("idx_temporal_ticker").on(table.ticker),
  })
);

export const aiFootnotesAnalysis = pgTable(
  "ai_footnotes_analysis",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    cacheKey: text("cache_key").notNull().unique(),
    companyName: text("company_name").notNull(),
  ...
  (table) => ({
    cacheKeyIdx: index("idx_footnotes_cache_key").on(table.cacheKey),
    tickerYearIdx: index("idx_footnotes_ticker_year").on(
      table.ticker,
      table.fiscalYear
    ),
  })
